...
import "./IWonderMarketplace.sol";

contract WonderMarketPlace is Ownable, IWonderMarketPlace {
    WonderFactory internal _WonderContract;
    ...
    modifier onlyTokenOwner(uint256 _tokenId) {
        require(
            msg.sender == _WonderContract.ownerOf(_tokenId),
            "not token owner"
        );
        _;
    }
    ...
    modifier marketApproved() {
        require(
            _WonderContract.isApprovedForAll(msg.sender, address(this)),
     ...
     */
    function setWonderContract(address _WonderContractAddress) public onlyOwner {
        _WonderContract = WonderFactory(_WonderContractAddress);
    }
     ...
     */
    function buyWonder(uint256 _tokenId) external payable activeOffer(_tokenId) {
        Offer memory offer = tokenIdToOffer[_tokenId];
        require(msg.value == offer.price, "payment must be exact");

        _executeOffer(offer);

        // tranfer Wonder ownership
        _WonderContract.transferFrom(offer.seller, msg.sender, _tokenId);
    ...
    activeOffer(_sireTokenId)
    {
        ...
        require(msg.value == offer.price, "payment must be exact");
        require(
            _WonderContract.readyToBreed(_matronTokenId),
            "matron on cooldown"
        );
        ...
        // set sire rites
        _WonderContract.sireApprove(_sireTokenId, _matronTokenId, true);
        uint256 kittenId = _WonderContract.breed(_sireTokenId, _matronTokenId);
        _WonderContract.transferFrom(address(this), msg.sender, kittenId);
